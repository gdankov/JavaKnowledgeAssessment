...
public class UserSession {
	...
	public List<QuestionAnswers> setUpLevel(int level) {
		List<QuestionAnswers> questionsAndAnswers = new ArrayList<>();
	...
	private List<QuestionAnswers> getQuestions(String topic, int level, int questionsPerTopic) {
		QuestionDAO questionDao = new QuestionDAO();
		
		List<QuestionAnswers> questions = questionDao.getAllQuestions(topic, level);
		List<QuestionAnswers> randQuestions = chooseRandomQuestions(questions, questionsPerTopic);
		return randQuestions;
	}
	...
	public boolean evaluateResult(List<Integer> results, List<QuestionAnswers> questions, int level) {
		int currentLevelScore = 0;
		
		int resultCounter = 0;
		for (QuestionAnswers question : questions) {
